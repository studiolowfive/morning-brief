...
export function selectReportSignals(signals, now = new Date()) {
  ...
  return signals
    ...
    .filter((signal) => {
      const productRelevance = Math.max(signal.scores.brandVoice, signal.scores.workflowLibrary, signal.scores.chainChasers);
      return signal.labels.includes("Hot") || signal.labels.includes("Conversation") || signal.labels.includes("Action Today") || productRelevance >= 3 || signal.scores.businessOpportunity >= 4;
    })
    .sort((a, b) => b.totalScore - a.totalScore)
    .slice(0, now.getDay() === 1 ? 14 : 10);
}
...
export function generateReport(scoredSignals, options = {}) {
  ...
  if (!selected.length) {
    lines.push("No high-confidence signals were collected for this report window. Check Connector Health above before treating the quiet as a market read.");
  } else if (llmBrief?.executiveSummary) {
    lines.push(llmBrief.executiveSummary);
  } else {
    const topTheme = themes[0]?.theme ?? "useful but fragmented conversations";
    lines.push(`The strongest read today is ${topTheme.toLowerCase()}. The best opportunities are not link-chasing; they are translating live market language into sharper positioning, useful content, and small product tests.`);
  }
  lines.push("");

  if (isMonday) {
    lines.push("## Weekend Catch-Up");
    lines.push(selected.length ? "Weekend signals are folded into today's ranking. Prioritize repeated themes and anything with Action Today attached." : "No weekend signals were available.");
    lines.push("");
  }

  lines.push("## Best Signals");
  selected.forEach((signal, index) => {
    const interp = interpretations?.get(signal.id);
    lines.push(`### ${index + 1}. ${signal.title}`);
    lines.push(`Labels: ${signal.labels.join(", ")}`);
  ...
  lines.push(themes.length ? themes.map((item) => `- ${item.theme}: ${item.count} signal(s)`).join("\n") : "- No clear repeated themes yet.");
  lines.push("");

  lines.push("## Audience Language");
  lines.push(audienceLanguage(selected) || "- Not enough conversational language collected yet. Add Reddit, LinkedIn, TikTok, or manual notes to improve this section.");
  lines.push("");
  ...
  if (llmBrief?.videoIdeas?.length) {
    ...
    lines.push("- Hook: Retro game feel is not pixels. It is friction, reward, and rhythm. Premise: connect Chain Chasers design choices to casual disc golf fun. Visual: quick gameplay beats and disc golf references. Fit: builds game taste and community.");
  }
  lines.push("");

  lines.push("## Reply / Engagement Opportunities");
  selected
    .filter((signal) => signal.connector === "reddit" || signal.labels.includes("Conversation"))
    .slice(0, 5)
    .forEach((signal) => {
  ...
  if (!selected.some((signal) => signal.connector === "reddit" || signal.labels.includes("Conversation"))) lines.push("- No specific reply targets collected today.");
  lines.push("");

  lines.push("## Product Marketing Opportunities");
  lines.push("Brand voice extraction: Package the pain as an operational problem: teams cannot scale content quality without shared voice standards.");
  lines.push("Workflow prompt library: Position it as better process for repeatable work, not a novelty prompt bundle.");
  lines.push("Chain Chasers: Watch for community language around casual play, nostalgia, courses, and mobile discovery; turn those into feature and launch hooks.");
  lines.push("");

  lines.push("## Worth Acting On Today");
  const today = selected.filter((signal) => signal.labels.includes("Action Today"));
  lines.push(today.length ? today.map((signal) => `- ${signal.title}: ${angleFor(signal)}`).join("\n") : "- Nothing has a strong enough timing reason to force action today.");
  lines.push("");

  lines.push("## Keep Watching");
  lines.push(selected.filter((signal) => signal.labels.includes("Watch")).slice(0, 5).map((signal) => `- ${signal.title}`).join("\n") || "- No watch-only items today.");
  lines.push("");

  lines.push("## Ignore For Now");
  lines.push("- Broad AI hype with no concrete buyer pain, repeatable workflow lesson, or audience language worth borrowing.");

...
function buildPolishPrompt(signals, dayStrength = "moderate", jobs = []) {
  ...
  const rolesField = profile
    ? `,\n  "roles_top_pick": {"opportunity": "the single best opportunity to act on TODAY (prefer fractional/contract)", "why": "one line on why this is the one to jump on now"},\n  "roles": [{"track": "A (contract lead) | B (role)", "opportunity": "what it is and where, in one line", "why_fit": "1 line tying it to Alex's profile", "action": "the concrete move (DM/apply/pitch)", "pitch_as": "OPTIONAL: if a full-time listing that's pitchable as contract/fractional, one line on how to reframe it; else omit", "link": "source url", "note": "ONLY if Track B and at/below the senior floor: 'borderline — your call' with the reason"}]`
    : "";
  const rolesInstruction = profile
    ? `\n\nROLES & GIGS: surface a curated shortlist of STRONG-FIT opportunities — aim for about 5 (up to 7), ranked best-fit first. Go below 5 only if the pool genuinely lacks that many strong fits; never pad with weak ones. "Strong fit" means real alignment with Alex's profile, and his fit is BROAD — do NOT narrow to prestigious Creative Director roles. Range across his actual range: Director/Head/VP of Brand, Senior Brand or Creative Strategist, Content Strategy Director, senior or contract copywriting, Fractional CMO/Head of Brand, AI-forward content leadership, and Track A contract leads (someone needing brand/voice/copy/strategy/fractional help in the MARKET SIGNALS). CURRENT PRIORITY: Alex is building his client roster, so RANK FRACTIONAL / CONTRACT / RETAINER opportunities FIRST and weight them heavily; full-time/permanent fits are secondary. REFRAME NUANCE: many full-time listings (esp. "strategic partner to the CEO," brand-build, 0-to-1, interim-flavored) can be pitched as contract/fractional — when a strong full-time listing fits, keep it and set "pitch_as" with how to reframe it, rather than dropping it. Track B = the JOB LISTINGS below: still respect the senior floor (drop clearly sub-senior; add a borderline "note" only for a genuinely worth-a-look case). ALSO choose "roles_top_pick": the single role best suited to Alex that makes sense to jump on today (prefer fractional/contract unless a full-time fit is clearly exceptional) — it must be one of the roles you surface. Follow the profile's HARD RULES EXACTLY, including any do-not-surface rules: never invent a listing, only use openings present here with their link. If there genuinely aren't strong fits, return fewer (or []) and a null top pick — do not manufacture.`
    : "";
...
export async function polishBrief(signals, { dayStrength = "moderate", jobs = [] } = {}) {
  ...
  try {
    ...
    const brief = {
      ...
      rolesTopPick: parsed.roles_top_pick && parsed.roles_top_pick.opportunity ? parsed.roles_top_pick : null
    };
